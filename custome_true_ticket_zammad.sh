...
#print lengkap pada kondisi if else paling bawah 
...
get_last_ticket_info() {
    ...
    state_id=$(echo "$curl_output" | jq -r ".[] | select(.title == \"$query\") | .state_id" | head -n 1)
...
create_ticket() {
    ...
    if [ "$(get_last_ticket_info)" != "$(state_id_open)" ]; then
        ...
        echo "$(date '+%Y-%m-%d %H:%M:%S') - Membuat tiket baru untuk : $TICKET_TITLE, Nomor Tiket: $TICKET_NUMBER, state hardcode: $(state_id_open), state id: $(get_last_ticket_info), Termonitor host $HOSTNAME - $HOSTSTATE" >> "$LOG_FILE"
    else
        # Jika tiket sudah ada dalam status "open", cetak pesan bahwa tiket tidak perlu dibuat lagi
        echo "$(date '+%Y-%m-%d %H:%M:%S') - Tiket sudah ada untuk Host $HOSTNAME dengan state $HOSTSTATE. Tidak membuat tiket baru. state hardcode: $(state_id_open), state id: $(get_last_ticket_info), Termonitor host $HOSTNAME - $HOSTSTATE" >> "$LOG_FILE"
    fi
}
